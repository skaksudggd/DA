...
def setup(db, token) -> dict:
    ...
    st, d = call("POST", "/api/cutting/orders", token, {
        ...
        "style_name": f"Gate F24 {STAMP}", "style_sku": f"VFH6B-{STAMP}",
...
def cleanup(db, ctx):
    ...
    counts = {
        ...
        "stok": db.rahaza_material_stock.delete_many({"material_id": mat.get("id")}).deleted_count,
        "ledger": db.rahaza_stock_ledger.delete_many({"material_id": mat.get("id")}).deleted_count,

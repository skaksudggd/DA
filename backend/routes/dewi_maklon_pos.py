...
async def _next_po_number(db, client_code: str) -> str:
    """Nomor PO Maklon — race-safe & formatnya bisa diatur owner
    (kunci `dewi_maklon_pos.po_number`, token {KLIEN})."""
    year = datetime.now(timezone.utc).year
    return await gen_prefixed_number(db, 'dewi_maklon_pos', 'po_number',
                                     f'MKL-{client_code}-{year}-', 4,

...
async def list_materials(request: Request, type: Optional[str] = None, search: Optional[str] = None,
    ...
    if low_stock and low_stock.lower() == "true":
        ...
        stock_docs = await db.rahaza_material_stock.find({}, {"_id": 0}).to_list(MASTER_FETCH_LIMIT)
        stock_by_mat = {}
        for s in stock_docs:
...
async def list_reorder_alerts(request: Request):
    ...
    for s in stock_docs:
        ...
        if mid:
            stock_by_mat[mid] = stock_by_mat.get(mid, 0) + float(s.get("qty") or 0)
    mats = await db.rahaza_materials.find({"active": True, "reorder_point": {"$gt": 0}}, {"_id": 0}).to_list(MASTER_FETCH_LIMIT)
    alerts = []
    for m in mats:
...
async def update_material(mid: str, request: Request):
    ...
    # FASE 6/7: harga satuan (HPP) — terima alias hpp/price, simpan ke unit_cost kanonik

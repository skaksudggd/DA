...
async def _capitalize_assets_from_grn(db, receipt_id: str, grn: dict, items: list, user: dict):
    ...
    for item in items:
        ...
        # Generate asset code (RC-5 fix: atomic race-safe numbering, was count_documents()+1)
        asset_code = await gen_prefixed_number(db, "rahaza_fixed_assets", "asset_code", "FA-", 5)

...
async def create_transfer(body: TransferCreate, user: dict = Depends(require_auth)):
    ...
    now = _now()
    tf_id = _uid()
    # Generate nomor referensi (RC-5 fix: atomic race-safe numbering, was count_documents()+1)
    ref_number = await gen_prefixed_number(
        db, "rahaza_bank_transfers", "ref_number",
        f"BT-{date.today().strftime('%Y%m%d')}-", 4)

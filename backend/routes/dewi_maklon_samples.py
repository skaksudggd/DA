...
async def _generate_sample_code(db, order_code: str) -> str:
    """Generate unique sample code: SMP-{ORDER_CODE}-{N} (RC-5 atomic, was count_documents()+1)"""
    return await gen_prefixed_number(db, "dewi_maklon_samples", "sample_code", f"SMP-{order_code}-", 2)

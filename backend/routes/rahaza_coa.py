...
# ── G4: dedup COA double-seed ────────────────────────────────────────────────
# SEED_TEMPLATE (4-digit) & DA_COA_SEED (3-digit) di-seed berdampingan (dua chart
# yang sama-sama dirujuk posting engine). 11 akun SEED_TEMPLATE yang NAMANYA sama
# dgn DA_COA_SEED TIDAK di-seed → kanonik = DA 3-digit (disetujui user).
# Dihitung by-name agar tetap sinkron bila template berubah.
def _coa_norm_name(n: str) -> str:
    return " ".join((n or "").strip().lower().split())


_DA_COA_NAMES = {_coa_norm_name(row[1]) for row in DA_COA_SEED}
DUP_TEMPLATE_CODES = {c for (c, name, *_rest) in SEED_TEMPLATE
                      if _coa_norm_name(name) in _DA_COA_NAMES}

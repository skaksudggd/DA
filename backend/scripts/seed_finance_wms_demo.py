...
def main() -> int:
    ...
    ROLLS = [
        # roll_no, material, warna, lot, supplier, po, panjang, berat, qc
        ("DEMO-RL-0001", "Cotton Combed 30s", "Navy", "LOT-A1", "PT Kain Sejahtera",
         "PO-DEMO-001", 120.0, 28.5, "pass"),
        ("DEMO-RL-0002", "Cotton Combed 30s", "Putih", "LOT-A1", "PT Kain Sejahtera",
         "PO-DEMO-001", 96.5, 22.0, "pass"),
        ("DEMO-RL-0003", "Fleece Cotton", "Hitam", "LOT-B7", "CV Tekstil Nusantara",
         "PO-DEMO-002", 60.0, 31.0, "pending"),
        ("DEMO-RL-0004", "Rayon Twill", "Maroon", "LOT-C3", "CV Tekstil Nusantara",
         "PO-DEMO-002", 18.0, 6.4, "partial"),
    ]
    try:
        cur = requests.get(f"{BASE}/api/wms/fabric-rolls", headers=h,
                           params={"limit": 200}, timeout=45).json()
        have_rolls = {str(x.get("roll_no")) for x in (cur.get("rolls") or [])}
    except Exception:
        have_rolls = set()
    for rn, mat, color, lot, sup, po, m, kg, qc in ROLLS:
        if rn in have_rolls:
            skip_n += 1
            continue
        try:
            body = {"roll_no": rn, "material_id": "", "material_code": rn.split("-")[-1],
            ...
            if rr.status_code in (200, 201):
                ...
                say(f"{R}✗{X} roll {rn}: HTTP {rr.status_code} {rr.text[:110]}")
                fail_n += 1
        except Exception as e:  # noqa: BLE001
            say(f"{R}✗{X} roll {rn}: {e}")
            fail_n += 1

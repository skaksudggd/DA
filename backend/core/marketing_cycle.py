...
def valid_period(period: str) -> Tuple[int, int]:
    """`YYYY-MM` → (tahun, bulan). Melempar ValueError bila bentuknya salah."""
    dt = datetime.strptime(str(period or '').strip(), '%Y-%m')
...
async def _auto_livehost(db, account_id: str, period: str) -> Dict[str, Any]:
    shifts = await db.marketing_livehost_shifts.find(
        ...
        {'_id': 0, 'host_id': 1, 'total_pay': 1},
    ).to_list(10000)
    total = sum(_num(s.get('total_pay')) for s in shifts)
    by_host: Dict[str, float] = {}
    for s in shifts:
        by_host[s.get('host_id')] = by_host.get(s.get('host_id'), 0.0) + _num(s.get('total_pay'))
    return {'amount': _r(total), 'basis': 'livehost_shifts',
            'evidence': f"{len(shifts)} shift terhitung", 'docs': len(shifts),
            'detail': [{'host_id': h, 'total_pay': _r(v)} for h, v in by_host.items()]}

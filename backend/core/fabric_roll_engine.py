...
# Satuan → uom roll yang dipakai `wh_fabric_rolls` (uom: kg | meter | yard).
...
def remaining_of(roll: dict) -> float:
    return _f(roll.get("remaining_kg") if roll.get("uom") == "kg" else roll.get("remaining_m"))


def remaining_field(roll: dict) -> str:
    return "remaining_kg" if roll.get("uom") == "kg" else "remaining_m"
...
async def create_rolls_from_receipt(db, receipt: dict, item: dict, material: dict,
    ...
    for ln in lines:
        ...
        doc = {
            ...
            "uom": uom,
            "length_m": qty if uom in ("meter", "yard") else 0.0,
            "weight_kg": qty if uom == "kg" else 0.0,
            "remaining_m": qty if uom in ("meter", "yard") else 0.0,
            "remaining_kg": qty if uom == "kg" else 0.0,

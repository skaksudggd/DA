...
export default function RahazaMaterialsModule({ token }) {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ code: '', name: '', type: 'fabric', unit: 'kg', category: '', category_name: '', composition: '', color: '', notes: '', min_stock: 0, min_stock_qty: '', min_stock_percentage: '', active: true, unit_cost: 0, gsm: '', width_cm: '', uoms: [], purchase_uom: '', issue_uom: '', display_uom: '', pack_unit: 'pack', pack_size: 1, display_in_packs: false });
    setFormError(''); setModalOpen(true);
  };
  const openEdit = (r) => {
    setEditing(r);
    setForm({ ...r, composition: readField(r, FIELD.composition), min_stock_qty: r.min_stock_qty || '', min_stock_percentage: r.min_stock_percentage || '', unit_cost: r.unit_cost || 0,
      ...
      {modalOpen && (
        <Modal onClose={() => setModalOpen(false)} title={editing ? `Edit ${editing.code}` : 'Material Baru'} size="md">
          <div className="space-y-3" data-testid="mat-form">
            ...
            {/* FASE 6/7: harga satuan (HPP) — dasar valuasi stok & jurnal persediaan */}
            <div>
              <label className="block text-xs font-medium text-foreground/70 mb-1">Harga Satuan / HPP (Rp)</label>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                Dipakai untuk <strong>nilai stok</strong> &amp; <strong>jurnal keuangan</strong> (penerimaan, pengeluaran,
                penyesuaian opname, scrap karantina). Kosong/0 = transaksi persediaan tidak bisa dinilai.
              </p>
            </div>

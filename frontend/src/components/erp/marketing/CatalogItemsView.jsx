...
export default function CatalogItemsView({
  ...
  const exportCsv = () => {
    ...
    const body = items.map((r) => [
      ...
      r.retail_price_master, `${r.hpp} (${r.hpp_source})`, r.margin, r.margin_pct,
                ...
                <tr key={r.id} className="border-t border-border hover:bg-muted/30"
                  ...
                  <td className="px-2.5 py-1.5 tabular-nums whitespace-nowrap">
                    ...
                    <span className="ml-1 text-[10px] text-muted-foreground">
                      ({r.hpp_source === 'rnd' ? 'R&D' : r.hpp_source === 'manual' ? 'manual' : 'belum ada'})
                    </span>
                  </td>
                  <td className="px-2.5 py-1.5 tabular-nums whitespace-nowrap">{rp(r.margin)}</td>
            ...
            {pg.paged.map((r) => (
              <div key={r.id} className="rounded-[var(--radius-md)] border border-border
                ...
                <div className="grid grid-cols-2 gap-1 text-[11px]">
                  ...
                  <span className="text-right tabular-nums">{rp(r.hpp)}</span>
                  <span className="text-muted-foreground">Marjin</span>
                  <span className="text-right tabular-nums">{rp(r.margin)} · {r.margin_pct}%</span>
